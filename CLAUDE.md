...
When encountering errors with the atopile MCP tools (e.g. `search_and_install_jlcpcb_part` returning `AssertionError`), always report the error to the user with the full details (tool name, parameters, error message) so we can file or resolve bugs. Do not silently work around tool failures without informing the user.
